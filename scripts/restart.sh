#!/bin/bash
# Night Watch's Window 统一环境变量管理与重启脚本
...
for var in "${REQUIRED_VARS[@]}"; do
  if ! grep -q "^$var=" "$ENV_OUT"; then
    echo "[ERROR] 关键环境变量 $var 缺失于 $ENV_OUT，重启中止。"
    exit 3
  fi
done
# 默认不重启数据库，仅重启 web/worker/redis/nginx
echo "[INFO] 重启 web/worker/redis/nginx 服务（不动数据库）..."
docker-compose restart web worker redis nginx

echo "[SUCCESS] 服务已重启，环境：$env"